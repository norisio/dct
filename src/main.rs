...
fn innerprod<I, J>(subimage: &I, costmpl: &J) -> f64 
  ...
  for y in 0..7{
    for x in 0..7 {
      result += (
        subimage.get_pixel(x, y).to_luma().data[0] *
...
fn main() {
  ...
  let mut costmpl : Vec<Vec<image::ImageBuffer<Luma<u8>, Vec<u8>>>> = Vec::new();

  for v in 0usize..7 {
    let mut costmpl_u : Vec<image::ImageBuffer<Luma<u8>, Vec<u8>>> = Vec::new();
    for u in 0usize..7 {
      costmpl_u.push( 
